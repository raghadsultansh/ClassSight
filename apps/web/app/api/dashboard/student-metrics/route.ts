...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${FASTAPI_URL}/dashboard/student-metrics`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error('FastAPI student-metrics error:', response.status, errorText)
    ...
    console.error('Dashboard student-metrics API error:', error)
    return NextResponse.json(
      { error: 'Failed to fetch student metrics data' }, 
      { status: 500 }
    )
  }
}
