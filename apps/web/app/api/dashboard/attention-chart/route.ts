...
export async function POST(request: Request) {
  try {
    ...
    // Use the correct FastAPI endpoint for attention chart
    const url = `${FASTAPI_URL}/dashboard/attention-chart`
    ...
    console.error('Attention Chart API error:', error)
    return NextResponse.json(
      { error: 'Failed to fetch attention chart data' },
      { status: 500 }
    )
  }
}
