...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${FASTAPI_URL}/dashboard/instructor-performance`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error('FastAPI instructor-performance error:', response.status, errorText)
    ...
    console.error('Dashboard instructor-performance API error:', error)
